 ...
 * minimum of one byte, but may be greater.
 */
typedef uint_least8_t byte;

typedef enum std_arena_flags {
  ARENA_STOP_REALLOC = 1 << 0,        // Whether the arena's backing memory
 ...
 * of [arena_create] when providing backing memory that is freed automatically,
 * such as a stack-allocated memory buffer.
 *
 ...
 * [ARENA_DISALLOW_REALLOC] is set or the arena is allocated on using
 * [arena_create_s], then if the arena would reallocate memory, it fails
 * instead. If an arena memory reallocation occurs and fails during this call
 * and [CONTINUE_ON_ALLOC_FAILURE] is set, the arena's [is_allocated] flag is
 * set to [false] and the function returns [NULL].
 */
void *arena_alloc(std_arena *arena, size_t size);

/**
 * De-allocates all memory initialized within an arena, allowing re-allocation
 * of previously allocated memory. Accessing any previous memory allocated
 * within an arena after calling [arena_clean] is undefined behaviour.
 */
